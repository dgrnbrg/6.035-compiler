...
*/
  ...
  static LowIrNode static_shortcircuit(LowIrValueBridge condBridge, LowIrNode trueNode, LowIrNode falseNode) {
    ...
    LowIrNode.link(condBridge.end, jumpNode)
    LowIrNode.link(jumpNode, trueNode)
    LowIrNode.link(jumpNode, falseNode)
  ...
  LowIrBridge destruct(ForLoop forloop) {
    ...
    def loopHasBreaks = forLoopBreakContinueStack[-1][2]
    ...
    landingPad.anno['loop-contains-breaks'] = loopHasBreaks
  ...
  LowIrBridge destruct(Break breakHiir) {
    ...
    forLoopBreakContinueStack[-1][2] << breakNode
